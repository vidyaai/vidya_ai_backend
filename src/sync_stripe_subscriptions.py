...
def sync_stripe_subscriptions():
    ...
    try:
        ...
        subscriptions_without_stripe_id = db.query(Subscription).filter(
            Subscription.stripe_subscription_id.is_(None)
        ).all()
        
        print(f"📊 Found {len(subscriptions_without_stripe_id)} subscriptions without Stripe IDs")
        ...
        for subscription in subscriptions_without_stripe_id:
            try:
                ...
                if not user or not user.email:
                    print(f"❌ User not found or no email for subscription {subscription.id}")
                    continue
                ...
                if not stripe_subscriptions.data:
                    print(f"❌ No Stripe subscriptions found for customer {stripe_customer.id}")
                    continue
                ...
                print(f"✅ Found Stripe subscription: {target_subscription.id} (status: {target_subscription.status})")
                ...
                # Update period dates
                if hasattr(target_subscription, 'current_period_start') and target_subscription.current_period_start:
                    subscription.current_period_start = datetime.fromtimestamp(
                        target_subscription.current_period_start
                    ).replace(tzinfo=timezone.utc)
                
                if hasattr(target_subscription, 'current_period_end') and target_subscription.current_period_end:
                    subscription.current_period_end = datetime.fromtimestamp(
                        target_subscription.current_period_end
                    ).replace(tzinfo=timezone.utc)
                
                subscription.cancel_at_period_end = getattr(target_subscription, 'cancel_at_period_end', False)
