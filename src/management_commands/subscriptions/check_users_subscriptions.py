...
def show_all_users_and_subscriptions():
    ...
    try:
            ...
            if subscriptions:
                ...
                for sub in subscriptions:
                    ...
                    plan = (
                        ...
                        .filter(PricingPlan.id == sub.plan_id)
                        .first()
                    )
                    ...
                    print(
                        f"       - Period Start: {sub.current_period_start or 'None'}"
                    )
        ...
        cancelled_subscriptions = (
            db.query(Subscription).filter(Subscription.status == "cancelled").count()
        )
        subscriptions_with_stripe = (
            db.query(Subscription)
            .filter(Subscription.stripe_subscription_id.isnot(None))
            .count()
        )
        subscriptions_without_stripe = (
            db.query(Subscription)
...
def show_subscription_summary():
    ...
    try:
        ...
        if rows:
            print(
                f"{'Email':<30} {'Plan':<15} {'Status':<10} {'Billing':<10} {'Stripe ID':<15} {'Cancel':<8} {'End Date':<12}"
            )
            print("-" * 120)
            ...
            for row in rows:
                ...
                print(
                    f"{email:<30} {plan_name:<15} {status:<10} {billing:<10} {stripe_id:<15} {cancel:<8} {end_date:<12}"
                )
        else:
            print("No subscriptions found.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--summary", action="store_true", help="Show quick summary only"
    )
    parser.add_argument(
        "--detailed", action="store_true", help="Show detailed view (default)"
    )
