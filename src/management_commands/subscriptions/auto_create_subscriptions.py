...
def create_missing_subscriptions():
    ...
    try:
        ...
        users_without_subs = (
            db.query(User)
            .filter(~User.id.in_(db.query(Subscription.user_id).distinct()))
            .all()
        )
        ...
        plus_plan = (
            ...
            .filter(PricingPlan.plan_key == "vidya_plus", PricingPlan.is_active == True)
            .first()
        )
        ...
        for sub in subscriptions:
            ...
            print(
                f"  • {user.email} -> {plan.name} ({sub.billing_period}) - {sub.status}"
            )
