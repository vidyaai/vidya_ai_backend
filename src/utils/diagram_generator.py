...
class DiagramGenerator:
    ...
    def __init__(self):
        ...
        self.ALLOWED_IMPORTS = {
            ...
            'numpy', 'np',
            'schemdraw', 'schemdraw.elements', 'schemdraw.logic',
            'networkx', 'nx',
            'graphviz',
            'math'
        }
    ...
    async def render_schemdraw(self, code: str) -> bytes:
        ...
        try:
            ...
            code = re.sub(
                r'(with\s+schemdraw\.Drawing\()\)',
                r'\1show=False)',
                code
            )

            # 3. Remove any plt.show() or d.show() calls that cause non-interactive errors
            code = re.sub(r'\bplt\.show\(\)', '# plt.show() removed for non-interactive', code)
            ...
            if re.search(dsave_pattern, code):
                ...
                code_with_output = re.sub(drawsave_pattern, f"drawing.save('{output_path}'", code)
        ...
        try:
            ...
            if diagram_type not in ['ai_image', 'svg']:
                try:
                    ...
                    logger.error(f"Fallback AI generation also failed: {str(fallback_error)}")

            return None

    async def generate_diagrams_batch(
        self,
        diagram_specs: List[Tuple[int, Dict[str, Any]]],
        assignment_id: str
        ...
        """
        try:
            logger.info(f"Starting batch diagram generation for {len(diagram_specs)} diagrams")

            async def process_diagram(question_index: int, spec: Dict[str, Any]) -> Tuple[int, Optional[Dict[str, Any]]]:
                ...
                try:
                    ...
                    if image_bytes is None:
                        logger.warning(f"Failed to generate diagram for question {question_index}")
                        return (question_index, None)

                    # Upload to S3
                    diagram_data = await self.upload_to_s3(image_bytes, assignment_id, question_index)
                    return (question_index, diagram_data)

                except Exception as e:
                    logger.error(f"Error processing diagram for question {question_index}: {str(e)}")
                    return (question_index, None)

            # Process diagrams in parallel with concurrency limit
            semaphore = asyncio.Semaphore(5)  # Max 5 concurrent renders

            async def bounded_process(q_idx: int, spec: Dict[str, Any]) -> Tuple[int, Optional[Dict[str, Any]]]:
            ...
            logger.info(f"Batch diagram generation complete: {success_count}/{len(diagram_specs)} successful")
