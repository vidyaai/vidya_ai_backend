...
class AssignmentDocumentParser:
    ...
    def _parse_with_standard_approach(
        ...
        prompt = self._create_extraction_prompt(document_text, file_name)
        ...
        response = self.client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert document parser. Your task is to extract existing assignment questions from documents and structure them properly for import into an assignment system. Do not create new questions - only extract what already exists in the document.",
                },
...
"""
        ...
        response = self.client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Extract assignment questions concisely. Keep responses brief to avoid token limits.",
                },
                {"role": "user", "content": prompt},
            ],
            response_format={"type": "json_object"},
    ...
    def _create_extraction_prompt(self, document_text: str, file_name: str) -> str:
...
4. For multiple choice questions, extract all provided options
