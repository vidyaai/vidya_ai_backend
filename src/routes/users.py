...
from models import (
    User, Folder, Video, Assignment, AssignmentSubmission,
    Course, CourseMaterial, CourseEnrollment, LectureSummary,
    SharedLink, Subscription, UserUsage,
)
...
async def delete_account(
    ...
    try:
        ...
        if active_sub and active_sub.stripe_subscription_id:
            try:
                ...
                logger.info(f"Cancelled Stripe subscription {active_sub.stripe_subscription_id} for user {firebase_uid}")
            except Exception as e:
                logger.warning(f"Failed to cancel Stripe subscription for user {firebase_uid}: {e}")
        ...
        db.query(SharedLink).filter(
            SharedLink.owner_id == firebase_uid
        ).delete(synchronize_session=False)

        # 3. Delete LectureSummaries + S3
        summaries = db.query(LectureSummary).filter(LectureSummary.user_id == firebase_uid).all()
        for s in summaries:
            _delete_s3_key(s.summary_pdf_s3_key)
        db.query(LectureSummary).filter(
            LectureSummary.user_id == firebase_uid
        ).delete(synchronize_session=False)
        ...
        db.query(Video).filter(Video.user_id == firebase_uid).delete(synchronize_session=False)
        ...
        for course in courses:
            materials = db.query(CourseMaterial).filter(CourseMaterial.course_id == course.id).all()
            for mat in materials:
                _delete_s3_key(mat.s3_key)
        db.query(Course).filter(Course.user_id == firebase_uid).delete(synchronize_session=False)
        ...
        assignments = db.query(Assignment).filter(Assignment.user_id == firebase_uid).all()
        for a in assignments:
            for f in a.uploaded_files or []:
                if isinstance(f, dict):
                    _delete_s3_key(f.get("s3_key"))
        db.query(Assignment).filter(Assignment.user_id == firebase_uid).delete(synchronize_session=False)
        ...
        db.query(Folder).filter(Folder.user_id == firebase_uid).delete(synchronize_session=False)
        ...
        db.query(UserUsage).filter(UserUsage.user_id == user_id).delete(synchronize_session=False)
        ...
        raise HTTPException(status_code=500, detail="Failed to delete account. No changes made.")
    ...
    try:
        ...
        logger.critical(f"DB deletion succeeded but Firebase Auth deletion FAILED for uid {firebase_uid}: {e}. Manual cleanup required.")
