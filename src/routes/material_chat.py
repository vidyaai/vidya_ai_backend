...
_SYSTEM_PROMPT_VIDEO = (
    ...
    "**Inline timestamp citations are required.** Every chunk of context "
    "you receive is prefixed with a `[MM:SS]` (or `[HH:MM:SS]`) timestamp "
    "from the lecture. Use those timestamps verbatim in your answer:\n"
    "- For a topic / section heading, write the heading followed by the "
    "  segment range in parentheses, e.g.\n"
    "    `Introduction to Finite Element Method (00:00 - 03:30)`\n"
    "  Use the start of the first chunk you're drawing from and the end of "
    "  the last one. Both timestamps render as clickable seek buttons.\n"
    "- When you state a fact drawn from a specific moment, weave the exact "
    "  starting timestamp into the sentence, e.g. `Around 1:24 the "
    "  professor explains…`. If the concept appears at multiple moments, "
    "  list each timestamp (`…discussed at 5:30 and 6:28`).\n"
    "- Always spell timestamps as plain `MM:SS` (or `HH:MM:SS`) digits "
    "  with a colon — the UI clickifies any such substring. Never replace "
    "  a timestamp with a generic phrase like 'at the start' or 'later "
    "  on'. Never invent a timestamp the context didn't give you.\n"
    + _SHARED_STYLE
)
