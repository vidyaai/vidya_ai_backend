...
def get_user_subscription(db: Session, user_id: str) -> Subscription:
    ...
    if not subscription:
        ...
        if user:
            ...
            logger.info(f"Created free subscription for user: {user_id} with plan: {subscription.plan.name if subscription.plan else 'None'}")
...
def check_usage_limits(db: Session, user_id: str, usage_type: str, video_id: str = None) -> dict:
    ...
    if usage_type == "video_per_day":
        ...
        if not video_id:
            return {"allowed": False, "reason": "Video ID required for question limit check"}
...
def increment_usage(db: Session, user_id: str, usage_type: str, amount: float = 1.0, video_id: str = None):
    ...
    if usage_type == "video_per_day":
        ...
        logger.info(f"Incremented videos_per_day for user {user_id}: +{amount} (total: {usage.videos_analyzed_today})")
        ...
        questions_per_video = usage.questions_per_video or {}
        questions_per_video[video_id] = questions_per_video.get(video_id, 0) + int(amount)
        usage.questions_per_video = questions_per_video
        logger.info(f"Incremented questions for user {user_id} on video {video_id}: +{amount} (total: {questions_per_video[video_id]})")
