...
def initialize_pricing_plans(db: Session):
    ...
    for plan_data in plans_data:
        ...
        existing_plan = db.query(PricingPlan).filter(
            PricingPlan.plan_key == plan_data["plan_key"]
        ).first()
...
def create_subscription(db: Session, user_id: str, plan_type: str, billing_period: str, 
                       stripe_subscription_id: str = None) -> Subscription:
    """Create a new subscription for user"""
    
    # Get the pricing plan
    plan = db.query(PricingPlan).filter(
        PricingPlan.plan_key == plan_type
    ).first()
    ...
    logger.info(f"Created subscription for user {user_id}: {plan_type} ({billing_period})")
    return subscription
...
def check_usage_limits(db: Session, user_id: str, usage_type: str) -> dict:
    ...
    if usage_type == "video_upload":
        ...
        if current >= limit:
            return {"allowed": False, "reason": f"Monthly video upload limit reached ({current}/{limit})"}
        ...
        if current >= limit:
            return {"allowed": False, "reason": f"Monthly YouTube chat limit reached ({current}/{limit})"}
        ...
        if current >= limit:
            return {"allowed": False, "reason": f"Monthly translation limit reached ({current:.1f}/{limit} minutes)"}
