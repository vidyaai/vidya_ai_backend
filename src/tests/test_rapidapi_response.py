        ...
        # Check for progress URL
            ...
            for attempt in range(5):  # Try 5 times (50 seconds)
                ...
                if progress_response.status_code == 200:
                    ...
                    print(
                        f"  - url: {progress_data.get('url', 'NOT FOUND')[:50] if progress_data.get('url') else 'NOT FOUND'}..."
                    )
                    print(
                        f"  - download_url: {progress_data.get('download_url', 'NOT FOUND')[:50] if progress_data.get('download_url') else 'NOT FOUND'}..."
                    )
                    ...
                    print(
                        f"❌ Progress check failed with status: {progress_response.status_code}"
                    )
        else:
