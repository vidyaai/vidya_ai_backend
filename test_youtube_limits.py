...
def test_youtube_limits():
    ...
    try:
        ...
        print(f"   Videos per day limit: {subscription.plan.features.get('videos_per_day', 0)}")
        print()
        ...
        for i, video_id in enumerate(TEST_VIDEOS, 1):
            ...
            if not existing_video:
                ...
                if usage_check["allowed"]:
                    print(f"   ✅ ALLOWED ({usage_check['current']}/{usage_check['limit']})")
                    ...
                    print(f"      Current: {usage_check.get('current')}/{usage_check.get('limit')}")
            else:
