...
tests = [
    ('D flip-flop circuit with AND gate and XOR gate, draw timing diagrams for 8 clock cycles', 'electrical'),
    ('3-bit shift register using D flip-flops with enable signal and decoder', 'electrical'),
...
for q, hint in tests:
    ...
    print(f'  Q: {q[:70]}')
    print(f'    domain={result["domain"]}  type={result["diagram_type"]}  tool={result["preferred_tool"]}  ai={result["ai_suitable"]}')
    print()

# Test _CODE_BETTER_TYPES
seq_types = ['sequential_circuit', 'flip_flop_circuit', 'counter_circuit',
             'shift_register', 'circuit_with_timing']
...
new_types = ['sequential_circuit', 'flip_flop_circuit', 'counter_circuit',
             'shift_register', 'cdc_diagram', 'circuit_with_timing', 'fsm_diagram']
for dt in new_types:
    key = ('electrical', dt)
